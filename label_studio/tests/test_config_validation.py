...
from label_studio.tests.utils import (
    make_task,
    make_annotation,
    make_prediction,
    project_id,
...
def test_change_label_config_repeater(
...
def test_parse_all_configs():
    ...
    result = [
        y
        for x in os.walk(folder_wildcard)
...
def test_label_config_versions(business_client, project_id):
    with io.open(
        os.path.join(
            os.path.dirname(__file__), 'test_data/data_for_test_label_config_matrix.yml'
        )
    ) as f:
