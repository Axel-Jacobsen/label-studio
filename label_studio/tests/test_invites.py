...
def test_signup_setting(business_client, client, settings):
    ...
    response = client.post(
        '/user/signup',
        data={'email': 'test_user@example.com', 'password': 'test_password'},
    )
    ...
    response = client.post(
        invite_url, data={'email': 'test_user@example.com', 'password': 'test_password'}
    )
...
def test_reset_token(business_client, client, settings):
    ...
    response = client.post(
        new_invite_url,
        data={'email': 'test_user2@example.com', 'password': 'test_password'},
    )
    assert response.status_code == 302
...
def test_token_get_not_post_shows_form(business_client, client, settings):
    ...
    invite_url = response.json()['invite_url']
    response = client.get(
        f'{invite_url}&email=test_user@example.com&password=test_password'
    )
