...
def test_svg_upload_sanitize(setup_project_dialog):
    ...
    endpoint = (
        f'/api/projects/{setup_project_dialog.project.id}/import?commit_to_project=true'
    )
    ...
    actual = (
        FileUpload.objects.filter(id=r.data['file_upload_ids'][0]).last().file.read()
    )
...
def test_svg_upload_do_not_sanitize(setup_project_dialog):
    ...
    assert "".join(xml_dirty.split()) == "".join(
