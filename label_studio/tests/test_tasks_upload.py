...
def post_data_as_format(setup, format_type, body, archive, multiply_files):
    ...
    if format_type == 'json_data':
        return setup.post(
            setup.urls.task_bulk, data=body, content_type="application/json"
        )

    # post as files
    if format_type == 'json_file':
        files = {
            f'upload_file{i}.json': io.StringIO(body) for i in range(0, multiply_files)
        }
    elif format_type == 'csv_file':
        files = {
            f'upload_file{i}.csv': io.StringIO(body) for i in range(0, multiply_files)
        }
    elif format_type == 'tsv_file':
        files = {
            f'upload_file{i}.tsv': io.StringIO(body) for i in range(0, multiply_files)
        }
    elif format_type == 'txt_file':
        files = {
            f'upload_file{i}.txt': io.StringIO(body) for i in range(0, multiply_files)
        }
    else:
...
def test_json_task_upload(
    setup_project_dialog, format_type, tasks, status_code, task_count, multiply_files
):
    ...
    r = post_data_as_format(
        setup_project_dialog, format_type, json.dumps(tasks), 'none', multiply_files
    )
    ...
    assert (
        Task.objects.filter(project=setup_project_dialog.project.id).count()
        == task_count * multiply_files
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            [{'data': {'dialog': 'Test'}, 'annotations': [{'result': [{'id': '123'}]}]}]
            * 10,
            201,
            10,
            10,
        ...
        (
            [
                {
                    'data': {'dialog': 'Test'},
                    'annotations': [{'result': [{'id': '123'}], 'ground_truth': True}],
                }
            ],
            201,
            1,
        ...
        (
            [{'data': {'dialog': 'Test'}, 'annotations': [{'trash': '123'}]}] * 10,
            400,
            0,
            0,
        ),
...
@pytest.mark.django_db
def test_json_task_annotation_and_meta_upload(
    setup_project_dialog, tasks, status_code, task_count, annotation_count
):
    ...
    annotations = Annotation.objects.filter(
        task__project=setup_project_dialog.project.id
    )
...
@pytest.mark.parametrize(
    ...
    [
            ...
            [
                {
                    'data': {'dialog': 'Test'},
                    'predictions': [
                        {'result': [{'id': '123'}], 'model_version': 'test'}
                    ],
                }
            ],
            201,
        ...
        (
            [{'data': {'dialog': 'Test'}, 'predictions': [{'WRONG_FIELD': '123'}]}],
            400,
            0,
            0,
        ),
...
def test_archives(
    ...
    r = post_data_as_format(
        setup_project_dialog, format_type, json.dumps(tasks), archive, multiply_files
    )
    print('Create json tasks with annotations result:', r.content)
    assert r.status_code == status_code, 'Upload one task with annotation failed'
...
def test_csv_tsv_task_upload(
    ...
    tasks = (
        tasks if format_type == 'csv_file' else tasks.replace(',', '\t')
    )  # prepare tsv file from csv
    r = post_data_as_format(
        setup_project_dialog, format_type, tasks, archive, multiply_files
    )
    print(f'Create {format_type} tasks result:', r.content)

    assert (
        r.status_code == status_code
    ), f'Upload one task {format_type} failed. Response data: {r.data}'
    assert (
        Task.objects.filter(project=setup_project_dialog.project.id).count()
...
def test_txt_task_upload(
    ...
    r = post_data_as_format(
        setup_project_dialog, format_type, tasks, 'none', multiply_files
    )
    print(f'Create {format_type} tasks result:', r.content)
...
@pytest.mark.django_db
def test_upload_duration(
    setup_project_dialog, tasks, status_code, task_count, max_duration
):
    """Upload JSON task with annotation to project"""
    r = post_data_as_format(
        setup_project_dialog, 'json_data', json.dumps(tasks), 'none', 1
    )
    print('Create json tasks with annotations result:', r.content)
    assert r.status_code == status_code, (
        'Upload one task with annotation failed',
        r.content,
    )
...
def test_url_upload(mocker, setup_project_dialog, tasks, status_code, task_count):
    ...
    with requests_mock.Mocker(real_http=True) as m:
        ...
        for task in tasks:
            assert (
                task.is_labeled
            ), 'Task should be labeled since annotation is ground_truth'
...
def test_upload_with_token(
    setup_project_for_token, tasks, status_code, task_count, bad_token
):
    ...
    assert (
        r.status_code == 201
    ), 'Create project result should be redirect to the next page: ' + str(r.content)
