...
def test_export(
    ...
    if aggregator_type == 'no_aggregation':
        ...
        if finished != '1':
            ...
            assert all(
                len(t['annotations']) == 0 for t in exports if t['id'] != task.id
            )
    else:
        assert (
            task_with_annotation['annotations'][0]['result'][0]['value']['choices'][0]
            == aggregated_class
        )
...
def test_export_with_predictions(
    ...
    for result in annotation_results:
        ...
        Annotation.objects.create(
            task=task, result=result, completed_by=business_client.admin
        )
    if predictions:
        for task in tasks:
            Prediction.objects.create(
                task=task, result=predictions['result'], score=predictions['score']
            )
