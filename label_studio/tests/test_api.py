...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "label_config": [
                    "toName=\"unexisted\" not found in names: ['my_class', 'my_text']"
                ]
            },
            400,
        ),
        ...
        (
            ...
            {
                'label_config': [
                    "Validation failed on : 'toName' is a required property"
                ]
            },  # noqa
            400,
        ),
...
def test_patch_project(
    client_and_token, configured_project, payload, response, status_code
):
    client, token = client_and_token
    payload['organization_pk'] = client.organization_pk
    r = client.patch(
...
def test_creating_activating_new_ml_backend(
    ...
    with requests_mock.Mocker() as m:
        ...
        m.post(
            f'{my_url}/setup',
            text=json.dumps({'model_version': 'Version from My Super AI'}),
        )
...
def test_patch_task(
    ...
    task = configured_project.tasks.order_by('-updated_at').all()[
        0
    ]  # call DB again after update
    response['project'] = configured_project.id
    response['created_at'] = task.created_at.isoformat().replace('+00:00', 'Z')
    response['updated_at'] = task.updated_at.isoformat().replace('+00:00', 'Z')
    response['id'] = task.id
