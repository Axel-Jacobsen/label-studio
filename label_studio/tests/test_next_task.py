...
def test_next_task_with_active_learning(
    ...
    for task, prediction, annotation in zip(tasks, predictions, annotations):
        ...
        Prediction.objects.create(
            task=task, model_version=project.model_version, **prediction
        )
        if annotation is not None:
            completed_by = (
                any_client.annotator
                if num_annotators == 1
                else annotator2_client.annotator
            )
            Annotation.objects.create(
                task=task, completed_by=completed_by, project=project, **annotation
            )
    r = any_client.get(f'/api/projects/{project.id}/next')
    assert r.status_code == status_code
    rdata = json.loads(r.content)
...
def test_active_learning_with_uploaded_predictions(business_client):
    ...
    tasks = [
        {
            'data': {'text': 'score = 0.5'},
            'predictions': [{'result': result, 'score': 0.5}],
        },
        {
            'data': {'text': 'score = 0.1'},
            'predictions': [{'result': result, 'score': 0.1}],
        },
        {
            'data': {'text': 'score = 0.3'},
            'predictions': [{'result': result, 'score': 0.3}],
        },
        {
            'data': {'text': 'score = 0.2'},
            'predictions': [{'result': result, 'score': 0.2}],
        },
        {
            'data': {'text': 'score = 0.4'},
            'predictions': [{'result': result, 'score': 0.4}],
        },
    ]
...
@pytest.mark.skipif(
    not redis_healthcheck(), reason='Multi user locks only supported with redis enabled'
)
@pytest.mark.parametrize(
    'sampling', (Project.UNIFORM, Project.UNCERTAINTY, Project.SEQUENCE)
)
@pytest.mark.django_db
...
def test_breadth_first_simple(business_client):
    ...
    # and completes it
    r = ann1.post(
        f'/api/tasks/{id1}/annotations/',
        data={'task': id1, 'result': annotation_result},
    )
    assert r.status_code == 201
...
def test_breadth_first_overlap_3(business_client):
    ...
    def complete_task(annotator):
        ...
        annotator.post(
            ...
            data={'task': task_id, 'result': annotation_result},
        )
        return task_id
...
def test_label_race_with_overlap(configured_project, business_client):
    ...
    r = ann2.post(
        f'/api/tasks/{overlap_id}/annotations/',
        data={'task': overlap_id, 'result': annotation_result},
    )
    assert r.status_code == 201
...
def test_fetch_final_taken_task(business_client):
    ...
    print(
        'ann2 takes the same task (because of depth-first) but just lock it - don\'t complete'
    )
    r = ann2.get(f'/api/projects/{project.id}/next')
...
def test_with_bad_annotation_result(business_client):
    ...
    bad_result = {
        ...
        'type': 'polygonlabels',
        'value': {
            'points': [[65.99824119670821, 73.11598603746282]],
            'polygonlabels': ['t11'],
        },
        'source': '$image',
        'to_name': 'img',
    ...
    for i in range(num_annotators):
        anns.append(
            make_annotator(
                {'email': f'ann{i}@testwithbadannotationresult.com'}, project, True
            )
        )
    ...
    with mock.patch('tasks.models.update_project_summary_annotations_and_is_labeled'):
        for i in range(10):
            task = make_task(
                {'data': {'image': f'https://data.s3.amazonaws.com/image/{i}.jpg'}},
                project,
            )
            for i in range(num_annotators):
                make_annotation(
                    {
                        'result': [bad_result] * 10 + [good_result] * 10,
                        'completed_by': anns[i].annotator,
                    },
                    task.id,
                )
    ...
    uncompleted_task = make_task(
        {'data': {'image': f'https://data.s3.amazonaws.com/image/uncompleted.jpg'}},
        project,
    )
    ...
    def make_async_annotation_submit(new_ann=None):
        ...
        if new_ann is None:
            new_ann = make_annotator(
                {'email': f'new_ann@testwithbadannotationresult.com'}, project, True
            )
        new_ann.post(
            f'/api/tasks/{task_id}/annotations/',
    ...
    assert (
        time.time() - t
    ) < 1, 'Time of annotation.submit() increases - that might be caused by redundant computations over the rest of the tasks - check that only a single task is affected by /api/tasks/<task_id>/annotations'  # noqa
...
def test_overlap_first(business_client, setup_before_upload, show_overlap_first):
    ...
    assert (
        Task.objects.filter(Q(project_id=project.id) & Q(overlap__gt=1)).count()
        == expected_tasks_with_overlap
    )
    ...
    all_tasks_with_overlap_are_labeled = all(
        t.is_labeled
        for t in Task.objects.filter(Q(project_id=project.id) & Q(overlap__gt=1))
    )  # noqa
    all_tasks_without_overlap_are_not_labeled = all(
        not t.is_labeled
        for t in Task.objects.filter(Q(project_id=project.id) & Q(overlap=1))
    )  # noqa
