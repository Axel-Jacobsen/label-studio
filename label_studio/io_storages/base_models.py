...
class StorageInfo(models.Model):
    ...
    last_sync = models.DateTimeField(
        _('last sync'), null=True, blank=True, help_text='Last sync finished time'
    )
    ...
    traceback = models.TextField(
        null=True, blank=True, help_text='Traceback report for the last failed sync'
    )
    meta = JSONField(
        'meta',
        null=True,
    ...
    def info_set_in_progress(self):
        ...
        if self.status != self.Status.QUEUED:
            raise ValueError(
                f'Storage status ({self.status}) must be QUEUED to move it IN_PROGRESS'
            )
    ...
    def health_check(self):
        ...
        if (
            self.status == self.Status.IN_PROGRESS
            and delta > settings.STORAGE_IN_PROGRESS_TIMER * 2
        ):
            self.status = self.Status.FAILED
            self.traceback = (
...
class Storage(StorageInfo):
    ...
    description = models.TextField(
        _('description'), null=True, blank=True, help_text='Cloud storage description'
    )
...
class ImportStorage(Storage):
    ...
    def add_task(
        ...
        if annotations:
            ...
            cancelled_annotations = len(
                [a for a in annotations if a.get('was_cancelled', False)]
            )
        ...
        with transaction.atomic():
            ...
            logger.debug(
                f'Create {storage.__class__.__name__} link with key={key} for task={task}'
            )
    ...
    def _scan_and_create_links(self, link_class):
        ...
        for key in self.iterkeys():
            ...
            self.info_update_progress(
                last_sync_count=tasks_created, tasks_existed=tasks_existed
            )
    ...
    def sync(self):
        if redis_connected():
            ...
            if not is_job_in_queue(
                queue, "import_sync_background", meta=meta
            ) and not is_job_on_worker(job_id=self.last_sync_job, queue_name='low'):
                ...
                logger.info(
                    f'Storage sync background job {sync_job.id} for storage {self} has been started'
                )
        else:
            try:
...
@job('low')
def import_sync_background(
    storage_class, storage_id, timeout=settings.RQ_LONG_JOB_TIMEOUT, **kwargs
):
...
def storage_background_failure(*args, **kwargs):
    ...
    if isinstance(args[0], rq.job.Job):
        ...
        if storage is None:
            logger.info(
                f'Storage {_class} {storage_id} not found at job {sync_job} failure'
            )
            return
...
class ExportStorage(Storage, ProjectStorageMixin):
    ...
    def sync(self):
        if redis_connected():
            ...
            self.info_set_job(sync_job.id)
            logger.info(
                f'Storage sync background job {sync_job.id} for storage {self} has been queued'
            )
        else:
            try:
                logger.info(f'Start syncing storage {self}')
...
class ImportStorageLink(models.Model):
    task = models.OneToOneField(
        'tasks.Task', on_delete=models.CASCADE, related_name='%(app_label)s_%(class)s'
    )
    ...
    def create(cls, task, key, storage):
        link, created = cls.objects.get_or_create(
            task_id=task.id, key=key, storage=storage, object_exists=True
        )
        return link
...
class ExportStorageLink(models.Model):
    ...
    updated_at = models.DateTimeField(
        _('updated at'), auto_now=True, help_text='Update time'
    )
    ...
    def create(cls, annotation, storage):
        link, created = cls.objects.get_or_create(
            annotation=annotation, storage=storage, object_exists=True
        )
        if not created:
