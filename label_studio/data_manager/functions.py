...
def get_all_columns(project, *_):
    ...
    if flag_set(
        'ff_back_2070_inner_id_12052022_short', user=project.organization.created_by
    ):
        result['columns'] += [
            {
                'id': 'inner_id',
    ...
    result['columns'] += [
        ...
        },
        {
            ...
            'schema': {
                'items': project.organization.members.values_list('user__id', flat=True)
            },
            'visibility_defaults': {'explore': True, 'labeling': False},
        },
        {
...
def preprocess_field_name(raw_field_name, only_undefined_field=False):
    ...
    field_name = (
        field_name[1:] if field_name[0] == '-' else field_name
    )  # remove direction
