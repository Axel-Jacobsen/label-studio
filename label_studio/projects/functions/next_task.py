...
def _try_breadth_first(tasks, user):
    ...
    tasks = tasks.annotate(
        annotations_count=Count(
            'annotations', filter=~Q(annotations__completed_by=user)
        )
    )
...
def _try_uncertainty_sampling(
    tasks, project, user_solved_tasks_array, user, prepared_tasks
):
...
def get_next_task(user, prepared_tasks, project, dm_queue, assigned_flag=None):
    ...
    with conditional_atomic():
        ...
        next_task, queue_info = postponed_queue(
            next_task, prepared_tasks, project, user, queue_info
        )
