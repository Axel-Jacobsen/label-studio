...
class BaseExportDataSerializer(FlexFieldsModelSerializer):
    ...
    def to_representation(self, task):
        ...
        if 'annotations' in self.fields:
            self.fields['annotations'].context[
                'interpolate_key_frames'
            ] = self.context.get('interpolate_key_frames', False)
...
class TaskFilterOptionsSerializer(serializers.Serializer):
    ...
    only_with_annotations = serializers.BooleanField(
        default=False, required=False, help_text=''
    )
...
class AnnotationFilterOptionsSerializer(serializers.Serializer):
    ...
    skipped = serializers.BooleanField(
        allow_null=True, required=False, help_text='Include skipped annotations'
    )
...
class SerializationOptionsSerializer(serializers.Serializer):
    ...
    predictions = SerializationOption(
        required=False, help_text='JSON dict with parameters'
    )
...
class ExportParamSerializer(serializers.Serializer):
    ...
    download_resources = serializers.BooleanField(
        default=settings.CONVERTER_DOWNLOAD_RESOURCES,
        help_text='Download resources in converter.',
        required=False,
    )
    # deprecated param to delete
    export_type = serializers.CharField(
        default='JSON', help_text='Export file format.', required=False
    )
...
class BaseExportDataSerializerForInteractive(
    InteractiveMixin, BaseExportDataSerializer
):
    pass
