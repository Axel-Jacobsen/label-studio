...
def parse_input_args(input_args):
    ...
    root_parser.add_argument(
        ...
        '--database',
        dest='database',
        help='Database file path for storing tasks and annotations',
    )
    root_parser.add_argument(
        '--data-dir',
        dest='data_dir',
        help='Directory for storing all application related data',
    )
    root_parser.add_argument(
        '-d',
        '--debug',
    ...
    root_parser.add_argument(
        '--password', dest='password', default='', help='Password for default user'
    )
    root_parser.add_argument(
        '--username', dest='username', default='', help='Username for default user'
    )
    root_parser.add_argument(
        '--user-token', dest='user_token', default='', help='User token for API access'
    )
    root_parser.add_argument(
        '--agree-fix-sqlite',
    ...
    parser_version = subparsers.add_parser(
        'version', help='Print version info', parents=[root_parser]
    )

    parser_user = subparsers.add_parser(
        'user', help='Print user info', parents=[root_parser]
    )
    ...
    parser_start = subparsers.add_parser(
        'start', help='Start Label Studio server', parents=[root_parser]
    )
    ...
    parser_reset_password = subparsers.add_parser(
        'reset_password',
        help='Reset password for a specific username',
        parents=[root_parser],
    )

    parser_shell = subparsers.add_parser(
        'shell', help='Run django shell', parents=[root_parser]
    )

    calculate_stats_all_orgs = subparsers.add_parser(
        'calculate_stats_all_orgs',
        help='Calculate task counters and statistics',
        parents=[root_parser],
    )
    calculate_stats_all_orgs.add_argument(
        ...
        default=False,
        action='store_true',
        help='Recalculate from scratch',
    )

    # export_project sub-command parser
    export_project = subparsers.add_parser(
        'export', help='Export project in a specific format', parents=[root_parser]
    )
    export_project.add_argument('project_id', help='Project ID')
    export_project.add_argument(
        'export_format', help='Export format (JSON, JSON_MIN, CSV, etc)'
    )
    export_project.add_argument(
        '--export-path', help='Export file path or directory', default=EXPORT_DIR
    )
    ...
    annotation_fill_updated_by = subparsers.add_parser(
        ...
        help='Fill the updated_by field for Annotations',
        parents=[root_parser],
    )
