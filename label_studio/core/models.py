...
class AsyncMigrationStatus(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        (
            STATUS_IN_PROGRESS,
            'Migration is in progress. Check meta for job_id or status.',
        ),
    ...
    status = models.CharField(
        max_length=100, choices=STATUS_CHOICES, null=True, default=None
    )
    ...
    def __str__(self):
        return (
            f'(id={self.id}) '
            + self.name
            + (' at project ' + str(self.project) if self.project else '')
        )
