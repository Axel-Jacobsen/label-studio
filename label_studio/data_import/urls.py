...
_api_projects_urlpatterns = [
    ...
    path(
        '<int:pk>/tasks/bulk/',
        api.TasksBulkCreateAPI.as_view(),
        name='project-tasks-bulk-upload',
    ),
    path('<int:pk>/import', api.ImportAPI.as_view(), name='project-import'),
    path(
        '<int:pk>/import/predictions',
        api.ImportPredictionsAPI.as_view(),
        name='project-import-predictions',
    ),
    path('<int:pk>/reimport', api.ReImportAPI.as_view(), name='project-reimport'),
    path(
        '<int:pk>/file-uploads',
        api.FileUploadListAPI.as_view(),
        name='project-file-upload-list',
    ),
]
...
urlpatterns = [
    ...
    path(
        ...
        api.UploadedFileResponse.as_view(),
        name='data-upload',
    ),
    path(
        'storage-data/uploaded/',
        api.DownloadStorageData.as_view(),
        name='storage-data-upload',
    ),
    path(
        'tasks/<int:task_id>/presign/',
        api.PresignStorageData.as_view(),
        name='storage-data-presign',
    ),
]
