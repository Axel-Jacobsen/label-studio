...
def async_import_background(import_id, user_id, **kwargs):
    ...
    (
        ...
        found_formats,
        data_columns,
    ) = load_tasks_for_async_import(project_import, user)
    ...
    if project_import.preannotated_from_fields:
        ...
        parsed_data = reformat_predictions(
            parsed_data, project_import.preannotated_from_fields
        )
    ...
    if project_import.commit_to_project:
        ...
        emit_webhooks_for_instance(
            user.active_organization, project, WebhookAction.TASKS_CREATED, tasks
        )
...
def reformat_predictions(tasks, preannotated_from_fields):
    ...
    for task in tasks:
        if 'data' in task:
            task = task['data']
        predictions = [
            {'result': task.pop(field)} for field in preannotated_from_fields
        ]
